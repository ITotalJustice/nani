...
class AniListButton extends Component {
  ...
  getEpisode(anilistData) {
    ...
    let {
      media: { episode_number: crunchyrollEpisode, media_id: id },
      collectionMedia,
    } = this.props;
    ...
    if (media && media.collection_name && loggedIn) {
      try {
        const data = await dispatch(
          getAniListItem(media.collection_name, media.collection_id)
        );

        const episode = this.getEpisode(data);
        const progress =
          data && data.mediaListEntry && data.mediaListEntry.progress;
  ...
  async updateAniList(e) {
    ...
    if (anilistItem && loggedIn && !updated) {
      ...
      try {
        ...
        dispatch(
          getAniListItem(media.collection_name, media.collection_id, true)
        );
