...
class Collection extends Component {
  render() {
    ...
    mediaIds = mediaIds.filter(
      (mediaId) => media[mediaId] && media[mediaId].available
    );
    ...
    return (
      <Fragment>
        {showTitle && title && (
          <TitleTag className="border-bottom pb-3 mb-4">
            <div className="d-flex justify-content-between">{title}</div>
          </TitleTag>
        )}
        <div className="row">
          {loading ? (
            [...Array(loadingCardsCount).keys()].map((index) => (
              <LoadingTag
                width={width}
                theme={theme}
                key={`loadingMediaCard-${index}`}
              />
            ))
          ) : mediaIds.length !== 0 ? (
            ...
            <div className="col-12 text-center">No items.</div>
          )}
        </div>
      </Fragment>
    );
