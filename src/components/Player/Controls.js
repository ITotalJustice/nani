...
class Controls extends Component {
  ...
  render() {
    ...
    const linkTitle = (
      <Link
        to={`/series/${media.series_id}#collection_${media.collection_id}`}
    ...
    return (
      ...
      >
        <div
          ...
          onDoubleClick={this.coverDoubleClick}
          onTouchEnd={this.coverTap}
        />
        ...
        <div className="episode-information text-white">
          ...
          {fullScreenView && (
            <h4>
              Episode {media.episode_number}: {media.name}
            </h4>
          )}
        </div>
        ...
        <div className="toolbar">
          ...
          {nextMedia && (
            <div
              className="toolbar-button cursor-pointer d-md-block d-none"
              title="Next Episode"
          ...
          <span className="text-white time-ticker">
          ...
          <span className="text-white time-ticker">
            {formatTime(duration - watchTime)}
          </span>

          <div
            className="toolbar-button cursor-pointer volume-icon"
            title="Volume"
          ...
          <Popover
            container={fullScreenView ? "player" : "body"}
            placement="top"
            trigger="legacy"
            isOpen={settingsOpen}
                  ...
                  <div className="col">
                    ...
                    >
                      ...
                      {levels.map((level, index) => (
                        <option key={`quality-${index}`} value={level}>
                          {level}p
                        </option>
                      ))}
                    </select>
          ...
          {!fullscreen && (
            <div
              className="toolbar-button cursor-pointer d-none d-md-block"
              title={isFullPage ? "Small" : "Full Page"}
              onClick={() => toggleFullPage()}
            >
              <FontAwesomeIcon
                icon={isFullPage ? "video" : "desktop"}
                size="lg"
              />
            </div>
          )}
