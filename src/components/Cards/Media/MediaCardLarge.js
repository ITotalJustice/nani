...
class MediaCardLarge extends Component {
  render() {
    ...
    const mediaSeries =
      media && series[media.series_id] ? series[media.series_id] : {};
    const imgFullURL =
      media && media.screenshot_image && media.screenshot_image.full_url;
    return (
      <div className={`col-12 col-sm-12 d-flex pb-4`}>
        ...
        >
          {media !== undefined && media.available ? (
            <Link
              to={`/series/${media.series_id}/${media.media_id}`}
                  ...
                  <CardImgOverlay className="pl-4 pr-4 pb-3 p-2 d-flex flex-row flex-wrap align-items-start">
                    ...
                    {media.duration ? (
                      <Badge color="secondary" pill>
                        {Math.ceil(media.duration / 60)} min
                      </Badge>
                    ) : null}
                  </CardImgOverlay>
                  <Progress
                    ...
                    value={Math.min(
                      100,
                      (media.playhead / media.duration) * 100
                    )}
                  />
                </div>
                  ...
                  <CardBody className="p-2 d-flex flex-column">
                    ...
                    <div className="mt-auto d-flex flex-row">
                      <Button
                        ...
                        color="primary"
                      >
                        <FontAwesomeIcon icon="info" /> Series Info
                      </Button>
                      <QueueButton
                        id={media.series_id}
                        size="sm"
