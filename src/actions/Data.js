    ...
    if (data) {
      switch (data.code) {
        case "bad_session": {
          ...
          const isMediaPage = matchPath(path, {
            path: "/series/:id/:media",
            exact: true,
          });
          if (!isMediaPage) {
            if (sessionId) {
        ...
        case "bad_request": {
          // when removed from the devices and the like
          await dispatch(logout(true));
...
export const addSeries = (obj) =>
  addIdCheckBulk("series_id", [obj], ADD_SERIES);
export const addSeriesBulk = (bulk) =>
  addIdCheckBulk("series_id", bulk, ADD_SERIES);

export const ADD_MEDIA = "ADD_MEDIA";
export const addMedia = (obj) => addIdCheckBulk("media_id", [obj], ADD_MEDIA);
export const addMediaBulk = (bulk) =>
  addIdCheckBulk("media_id", bulk, ADD_MEDIA);

export const ADD_COLLECTION = "ADD_COLLECTION";
export const addCollection = (obj) =>
  addIdCheckBulk("collection_id", [obj], ADD_COLLECTION);
export const addCollectionBulk = (bulk) =>
  addIdCheckBulk("collection_id", bulk, ADD_COLLECTION);
...
export const addSeriesCollection = (obj) =>
  addIdArr(obj, ADD_SERIES_COLLECTION);
...
export const getHistory = (
  ...
  if (
    state.Data.history &&
    state.Data.history.data.length > 0 &&
    !force &&
    !append
  )
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!append) {
        ...
        dispatch(setHistory(offset, [...state.Data.history.data, ...data]));
      }
      batch(() => {
        dispatch(addSeriesBulk(data.map((d) => d.series)));
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      batch(() => {
        ...
        dispatch(
          addSeriesCollection({ id, arr: data.map((d) => d.collection_id) })
        );
      });
      resolve(data);
    } catch (err) {
...
export const updateSeriesQueue = ({ id, inQueue }) => (dispatch, getState) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const resp = await api({
        method: "post",
        route: inQueue ? "remove_from_queue" : "add_to_queue",
        data: form,
        locale: state.Options.language,
        noCancel: true,
      });
...
export const getSeriesList = (filter = "simulcast", noCancel = false) => (
  dispatch,
  getState
) => {
  const state = getState();
  const params = {
    ...
    media_type: "anime",
    fields:
      "series.series_id,series.in_queue,series.name,series.media_count,series.portrait_image",
    limit: 54,
    offset: 0,
    filter,
  };
...
export const getCategories = () => (dispatch, getState) => {
  ...
  if (Object.keys(state.Data.categories).length)
    return Promise.resolve(state.Data.categories);
...
export const getAniListItem = (name, collectionId, force = false) => (
  dispatch,
  getState
) => {
  const state = getState();

  if (state.Data.anilist[collectionId] && !force)
    return Promise.resolve(state.Data.anilist[collectionId]);

  if (!state.Auth.anilist.token)
