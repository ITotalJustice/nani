...
class Media extends Component {
  ...
  async loadVideo() {
    ...
    try {
      ...
      if (!isCancel(err)) {
        this.setState({
          error: (err && err.data && err.data.message) || "An error occurred.",
        });
        console.error(err);
      }
    }
  ...
  getVideoData() {
    ...
    const currentCollection =
      mediaObj && collectionMedia[mediaObj.collection_id];

    // grab the ids for the next or previous episode
    const nextEpisodeId =
      currentCollection &&
      currentCollection[currentCollection.indexOf(mediaObj.media_id) + 1];
    const prevEpisodeId =
      currentCollection &&
      currentCollection[currentCollection.indexOf(mediaObj.media_id) - 1];
    const nextEpisodeMedia = nextEpisodeId && media[nextEpisodeId];
    const prevEpisodeMedia = prevEpisodeId && media[prevEpisodeId];
    const nextEpisodeImgURL =
      nextEpisodeId &&
      nextEpisodeMedia.screenshot_image &&
      nextEpisodeMedia.screenshot_image.full_url;
    const prevEpisodeImgURL =
      prevEpisodeMedia &&
      prevEpisodeMedia.screenshot_image &&
      prevEpisodeMedia.screenshot_image.full_url;

    const imgFullURL =
      mediaObj &&
      mediaObj.screenshot_image &&
      mediaObj.screenshot_image.full_url;
  ...
  render() {
    ...
    const loadedVideo =
      ...
      streamData.media_id === mediaId;

    // remove episodes that are before the one currently being watched
    const nextEpisodes =
      mediaObj && currentCollection
        ? currentCollection.filter(
            (collectionMediaId) => Number(collectionMediaId) > Number(mediaId)
          )
        : false;
    ...
    const streams =
      (streamData &&
        ...
        streamData.stream_data.streams) ||
      [];
    ...
    const containedPlayer = (
      <div
        className={
          fullPage
            ? ""
            : "player-width position-relative embed-responsive embed-responsive-16by9"
        }
      >
        <div
          className={
            fullPage
              ? "position-relative vw-100 vh-100 overflow-hidden"
          ...
          <div className="col-sm-12">
            <PlayerContainer fullWidth={fullPage}>
              <div
                className={
                  !fullPage
                    ? "d-flex mb-4 mt-2 justify-content-center position-relative"
                    : ""
                }
              >
                {prevMedia && !fullPage ? (
            ...
            <h5 className="d-flex flex-column flex-md-row flex-wrap">
              ...
              {mediaObj.episode_number ? (
                <Badge color="secondary" className="mr-md-2 mb-1 badge-outline">
                  <FontAwesomeIcon icon="list-ol" /> Episode{" "}
                  {mediaObj.episode_number}
                </Badge>
              ) : null}
              <Badge color="info" className="mr-md-2 mb-1 badge-outline">
                <FontAwesomeIcon icon="clock" /> {formatTime(mediaObj.duration)}
              </Badge>
              <Badge
                color="secondary"
                className="mr-md-2 mb-1"
                tag="a"
                target="_blank"
                rel="noopener noreferrer"
                href={`
                      https://www.crunchyroll.com/media-${mediaObj.media_id}
                    `}
              >
                <FontAwesomeIcon icon="search" /> Open on Crunchyroll
              </Badge>
              {currentCollection ? (
                <Fragment>
                  <AniListButton
                  ...
                  collectionMedia[mediaObj.collection_id]
                    ? // get all the next episodes
                      collectionMedia[mediaObj.collection_id].slice(
                        collectionMedia[mediaObj.collection_id].indexOf(
                          mediaId
                        ) + 1
                      )
                    : []
    ...
    return (
      <Fragment>
        <Helmet defer={false}>
          <title>
            {loadedDetails
              ? `Episode ${mediaObj.episode_number}: ${mediaObj.name} - ${mediaObj.collection_name}`
              : "Loading..."}
          </title>
        </Helmet>
        {error && (
          <h1 className="col-sm-12 text-center text-danger">{error}</h1>
        )}
        {defaultView}
      </Fragment>
    );
