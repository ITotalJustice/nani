...
class Dashboard extends Component {
  ...
  render() {
    ...
    const continueIds = [...uncompletedHistory, ...queueIds].slice(
      0,
      continueCount
    );
    // grab all the series from the queue
    const queueSeries = queue.map((item) => item.series).slice(0, 6);
    ...
    const mostPopular =
      list && list["popular"] ? list["popular"].list.slice(0, cardCount) : [];
